...
Produce a self-hosting compiler flow for the current Cy bootstrap architecture.

## What Is Implemented

1. Self-hosting compiler source in `.cy`
- File: `compiler/v3_seed.cy`
- Emits standalone C compiler source from `compiler/v3_compiler_template.c`

2. Compiler binary modes
- Normal mode: compile `.cy` source into direct C output
- Self mode: `--emit-self` emits the compiler's own C source (`__FILE__` copy)

3. Deterministic rebuild loop
- Stage1: runtime (`./cy`) -> `compiler_stage1.c`
